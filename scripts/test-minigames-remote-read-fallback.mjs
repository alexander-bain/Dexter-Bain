...
const remoteServer = http.createServer((_request, response) => {
...
const tempDirectory = await fs.mkdtemp(path.join(os.tmpdir(), "minigames-read-test-"));
...
await fs.writeFile(
  ...
  JSON.stringify({
    games: {
      "backup-game": {
        entries: [
          { name: "Dex", picks: ["winner"], savedAt: "2026-01-01T00:00:00.000Z" },
        ],
        rooms: {},
        results: {},
      },
...
try {
  ...
  const response = await fetch(
    `http://127.0.0.1:${backendPort}/api/minigames/backup-game/entries`
  );
  const body = await response.json();

  assert.equal(response.status, 200);
  assert.equal(body.entries.length, 1);
  assert.equal(body.entries[0].name, "Dex");
  assert.deepEqual(body.entries[0].picks, ["winner"]);
  console.log("Minigames remote-read fallback test passed.");
} catch (error) {
