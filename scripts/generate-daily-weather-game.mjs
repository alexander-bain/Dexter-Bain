...
function localHeadlineAnswers({ weekend, likelyWeatherLead, likelySportsLead, likelyMarketUp }) {
  if (weekend) {
    return [
      { label: "Weather", odds: likelyWeatherLead ? 34 : 26, id: "weather" },
      { label: "Sports", odds: likelySportsLead ? 28 : 22, id: "sports" },
      { label: "Traffic", odds: 20, id: "traffic" },
      { label: "Local issues", odds: 18, id: "local-issues" }
    ];
  }

  return [
    { label: "Weather", odds: likelyWeatherLead ? 34 : 26, id: "weather" },
    { label: "Stocks", odds: likelyMarketUp ? 28 : 22, id: "stocks" },
...
function dayWatchEvent(date, forecast) {
  ...
  const newsQuestions = [
    choiceQuestion({
      ...
      answers: localHeadlineAnswers({ weekend, likelyWeatherLead, likelySportsLead, likelyMarketUp })
    }),
    yesNoQuestion({
