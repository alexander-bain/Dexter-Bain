...
function dayWatchEvent(date, forecast) {
  ...
  const likelyGasUp = high >= 74 || windSpeed >= 10;
  const likelyMusicChanged = includesAny(skyText, ["rain", "shower", "thunderstorm"]) || high >= 68;
  const likelySportsLive = !weekend || includesAny(skyText, ["sunny", "clear", "partly"]);
  const likelyWeatherLead = sky === "rain-likely" || (sky === "mostly-cloudy" && !likelyMarketUp);
  const likelySportsLead = likelySportsLive && windSpeed < 18;
  const localHeadlineAnswers = weekend
    ? [
        ...
        { label: "Local events", odds: 22, id: "local-events" }
      ]
    : [
  ...
  const selectedQuestions = [
    ...
    yesNoQuestion({
      text: "By 5 PM, will the wind be stronger than 15 mph?",
