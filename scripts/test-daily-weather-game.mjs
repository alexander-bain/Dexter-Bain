...
async function run() {
  ...
  try {
        ...
        (async () => {
          ...
          const botPicks = game.questions.map((question, index) => {
            ...
            const risks = answers
              ...
              .sort((left, right) => right.points - left.points || left.odds - right.odds);
            const choice = index % 2 === 1 && risks.length
              ? risks[0]
              : index % 5 === 4 && risks[1]
              ? risks[1]
              : favorites[0];
          ...
          while (Date.now() - start < 15000) {
            ...
            const entryCountValue = Number(entryCount.trim() || "0");
            if (
    ...
    if (riskPickCount < 2) {
      throw new Error(`Saved picks did not take enough risks: ${JSON.stringify({ riskPickCount, botPickMap, favoritePickMap })}`);
    }
