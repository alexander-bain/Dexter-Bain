      ...
      expression: `
        (async () => {
          ...
          const botPicks = game.questions.map((question, index) => {
            ...
            const valuePicks = answers.slice().sort((left, right) => {
            ...
            const risks = answers
              .filter((candidate) => candidate.odds <= 55 || answers.length <= 2)
              .sort((left, right) => right.points - left.points || right.odds - left.odds);
            const choice = index % 5 === 1
              ? (valuePicks[0] || favorites[0])
              : index % 5 === 3
              ? (risks[0] || valuePicks[0] || favorites[0])
              : index % 5 === 4
              ? (risks[1] || risks[0] || valuePicks[0] || favorites[0])
              : favorites[0];
